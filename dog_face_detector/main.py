...
# Load Models
detector = dlib.cnn_face_detection_model_v1('assets/data/dogHeadDetector.dat')
predictor = dlib.shape_predictor('assets/data/landmarkDetector.dat')

# Load Dog Image
img_path = 'assets/images/18.jpg'
...
cv2.imwrite('assets/images//%s_out%s' % (filename, ext), img_out)
plt.figure(figsize=(16, 16))
plt.imshow(img_result)
...
horns = cv2.imread('assets/images/horns2.png', cv2.IMREAD_UNCHANGED)
horns_h, horns_w = horns.shape[:2]

nose = cv2.imread('assets/images/nose.png', cv2.IMREAD_UNCHANGED)
...
for shape in shapes:
    ...
    img_result2 = overlay_transparent(img_result2, nose, nose_center[0], nose_center[1],
...
cv2.imwrite('assets/images/%s_out2%s' % (filename, ext), img_out2)
plt.figure(figsize=(16, 16))
